        ...
        CardPlay play)
    {
        if (CombatState is not null)
        {
            ...
            List<CardPileAddResult> added = [
                await CardPileCmd.AddGeneratedCardToCombat(statuses[0], PileType.Draw, Owner),
                await CardPileCmd.AddGeneratedCardToCombat(statuses[1], PileType.Draw, Owner),
                await CardPileCmd.AddGeneratedCardToCombat(statuses[2], PileType.Draw, Owner)
            ];
        ...
        await PowerCmd.Apply<StrengthPower>(choiceContext, Owner.Creature, gainAndDraw, Owner.Creature, this);
